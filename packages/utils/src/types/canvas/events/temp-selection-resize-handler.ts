...
export class TempSelectionResizeEventHandler implements IElementEventHandler {
    onMouseDown(e: MouseEvent, board: CanvasBoard, ctx: CanvasRenderingContext2D) {
        ...
        board.ActiveObjects.forEach((ele) => {
            // ele.resize(ctx, { dx: 0, dy: 0 }, board.CursorPosition!, "down", false);
        ...
        board.ActiveObjects.forEach((ele) => {
            ...
            switch (type) {
                ...
                case ElementEnum.Square: {
                    const { x: ex = 0, y: ey = 0, h: eh = 0 } = value;
                    const uh = (eh * rh) / ph;
                    const uw = (eh * rw) / pw;
                    let ox = 0;
                    let oy = 0;
            ...
            board.ActiveObjects.forEach((ele) => {
                ...
                switch (type) {
                    ...
                    case ElementEnum.Rectangle: {
                        ...
                        //     ele.update(ctx, { h: uh, w: uw, x: ux, y: uy, points: [] }, "move", false);
                        // }
                        ele.updateValue(ctx, { h: uh, w: uw, x: ux, y: uy }, "up", false);
                        break;
                    }
                    case ElementEnum.Square: {
                        const { x: ex = 0, y: ey = 0, h: eh = 0 } = value;
                    ...
                    default:
                        ele.updateValue(ctx, { ...ele.Value, points: [[offsetX, offsetY]] }, "up", false);
                }
            });
            board.saveBoard();
        }
