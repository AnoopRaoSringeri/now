...
import { CanvasActionEnum, ElementEnum } from "@now/utils";
            ...
            if (e.detail === 1) {
                if (this.Board.HoveredObject) {
                    ...
                    if (this.Board.SelectionElement) {
                        ...
                        this.Board._currentCanvasAction = CanvasActionEnum.Select;
                        this.Board.TempSelectionArea = CavasObjectMap[ElementEnum.Rectangle](
                            {
                                x: offsetX,
                                y: offsetY,
                                h: 0,
                ...
                if (this.Board.TempSelectionArea) {
                    if (this.Board._currentCanvasAction === CanvasActionEnum.Select) {
                        ...
                        const cp = this.Board.Helper.getCursorPosition(
                            { x: offsetX, y: offsetY },
                            {
                                id: this.Board.TempSelectionArea.id,
                                x: rx,
                                y: ry,
                                h: rh,
                                w: rw,
                                type: this.Board.TempSelectionArea.type
                            },
                            this.Board.TempSelectionArea.type
                        );
                        this.Board.ActiveObjects.forEach((ele) => {
    ...
    onMouseUp(e: MouseEvent) {
        ...
        if (this.Board.TempSelectionArea) {
            if (this.Board._currentCanvasAction === CanvasActionEnum.Resize) {
                ...
                const { h = 0, w = 0, x: sax = 0, y: say = 0 } = this.Board.TempSelectionArea.getValues();
                ...
                this.Board.TempSelectionArea.set("ShowSelection", true);
            }
            this.Board.SelectionElement = this.Board.TempSelectionArea;
        }
